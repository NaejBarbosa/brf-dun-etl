...
# -*- coding: utf-8 -*-
"""
Nome: export_all_xlsx.py
Descrição: Exporta TODO o catálogo de produtos BRF do SQLite para um
           arquivo Excel XLSX com hiperlinks de imagens na pasta de downloads do Android.
"""
...
FILENAME = 'brf_todos_produtos.xlsx'
...
def main():
    ...
    # Query selecionando todos os produtos da tabela
    query = f"SELECT {', '.join(headers)} FROM produtos"
    cursor.execute(query)
    rows = cursor.fetchall()
    
    if not rows:
        print("[!] Aviso: Nenhum produto cadastrado no banco.")
        conn.close()
        return
        
    print(f"[*] Exportando todo o catálogo ({len(rows)} produtos) para XLSX...")
    
    wb = Workbook()
    ws = wb.active
    ws.title = "Todos os Produtos BRF"
